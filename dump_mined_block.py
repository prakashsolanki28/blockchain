...
# generate a block
class Block:
    # max num till we will check
    nonce_limit = 100000000000
    # no. of zeros in beginning
    zeros = 4
    # properties of block
    ...
    def calc_hash(self):
        for nonce in range(self.nonce_limit):
            base_text = str(self.index) + str(self.timestamp) + str(self.data) + str(self.prev_hash) + str(nonce)
            hash_try = hashlib.sha256(base_text.encode()).hexdigest()
            if hash_try.startswith('0' * self.zeros):
                self.nonce = nonce
                return hash_try
        return -1
